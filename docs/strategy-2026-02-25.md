...
| Phase | Status |
|---|---|
| Phase 0: Normalize | DONE — v024 active, 29,247 records |
| Phase 0: Expand | DONE — v007 active, 64,244 address entries |
| Phase 0: Audit metrics | NOT DONE |
| Phase 1: Parcels — Parcelled stage | IN PROGRESS — engine built, sandbox tested with live provincial API, auto token refresh working |
| Phase 1: Parcels — Compiled stage | DONE — v002 active, 29,247 records compiled |
| Phase 1: Parcels — Full parcelled run | PENDING — need complete run with all sources through provincial API |
| Phase 2: Party grouping | NOT STARTED |
| Phase 3: App reorg | NOT STARTED |
| Phase 4: Integrations | NOT STARTED |

### March 2 Progress

**Parcelled stage built.** New versioned pipeline stage (`cleo parcelled`) resolves every source record to a provincial assessment parcel. Resolution priority: ARN direct (high confidence) → PIN bridge (medium) → spatial/coords (low) → none. Auto-fetches AgMaps token via headless browser automation. Handles mid-run token expiry with automatic refresh.

**Compiled stage built.** New versioned pipeline stage (`cleo compile`) merges all upstream data into one record per source ID. RT records get transaction bypass + addresses + geocode + parcel. GW and Brand records get addresses + geocode + parcel.

**Pipeline architecture formalized.** Full 8-stage pipeline documented in `docs/pipeline-with-parcelled.md`:
Parse → Normalize → Expand → Geocode → Parcelled → Compile → Properties (read-only).

**OSM POI integration planned.** OSM POIs will enter at the Parcelled stage with native coords (skip parse/normalize/expand/geocode). Each gets an `OSM_XXXXX` ID.

**Provincial API is free.** No cost for queries — only time cost (0.4s throttle per request).
...
- **Realtrack ingestion** — ~92,547 HTML files across 9 property types (retail, farm, multifamily, comm-ind-land, office, hotel-motel, other-bldg, other-land, restaurant-bar).
...
- **Geocoding** — ~50K entries in unified multi-provider coordinate store (Mapbox, HERE, Geocodio, scraper). Legacy geocode cache (~36K) still maintained. Daily Geocodio cron (2,300/day free tier) runs via launchd plist.
...
**5. Parcel consolidation — partially resolved** (Addressed: parcelled stage built with provincial GIS API integration. Auto-fetches AgMaps token. Resolution chain: ARN → PIN → coords → provincial API. Parcel cache grows permanently. Full pipeline run pending.)

**6. Address normalization is cracking under pressure** (Addressed: normalize stage v019 now handles all 3 sources — RT, brands, GW — through a single canonical path via `cleo/normalize/address.py`. Expand stage v004 produces 64,244 decomposed address entries from 29,247 normalized records.)
...
> **Status (March 2):** Parcelled stage implemented. Provincial API integrated with auto-token refresh. Compiled stage built. Need full parcelled run, then rewire properties to use compiled/parcelled ARNs.
...
| Path | What | Count |
|---|---|---|
| `data/html/{type}/*.html` | Raw Realtrack HTML | ~92,547 files across 9 types |
...
| `data/geocode_cache.json` | Address → lat/lng | ~36K entries (legacy) |
...
| `data/seen_rt_ids.json` | Master RT ID tracker | ~92,547 IDs |
| `brands/data/*.json` | Brand scraper output | Multiple brand files |
| `data/coordinates.json` | Unified multi-provider geocode store | ~50K entries |
| `data/parcelled/active/*.json` | Parcelled resolution records | ~29,247 (when complete) |
| `data/compiled/active/*.json` | Compiled merged records | 29,247 records (v002) |
| `data/parcels/parcel_cache.json` | Provincial parcel cache (permanent, keyed by 20-digit ARN) | Growing |
