...
def _collect_buyer_section_lines(soup) -> List[Tuple[str, bool]]:
    transferee_tag = soup.find("font", string=re.compile(r"Transferee\(s\)"))
    if not transferee_tag:
        return []
    boundary = soup.find("font", string=re.compile(r"Description"))
    if boundary is None:
        boundary = soup.find("font", string=re.compile(r"Site"))
    stop_paragraph = boundary.parent if boundary else None
    br_tag = transferee_tag.find_next("br")
    if not br_tag:
        return []

    section_paragraph = transferee_tag.find_parent("p")
...
def parse_buyer_structured(soup) -> Dict[str, List[str]]:
    lines = _collect_buyer_section_lines(soup)
    result = {
        "BuyerStructuredCompanyLines": [],
        "BuyerStructuredContactLines": [],
        "BuyerStructuredAddressLines": [],
    }
    ...
    for raw, from_em in lines:
        ...
        if not first_company_added:
            result["BuyerStructuredCompanyLines"].append(line)
            first_company_added = True
            last_added = ("company", len(result["BuyerStructuredCompanyLines"]) - 1)
            continue

        lowered = line.lower()
        is_contact, contact_value = strip_contact_prefix(line)
        if is_contact and contact_value:
            result["BuyerStructuredContactLines"].append(contact_value)
            address_mode = True
            last_added = ("contact", len(result["BuyerStructuredContactLines"]) - 1)
            continue
            ...
            if last_added and last_added[0] == "address":
                ...
                if 0 <= idx < len(address_entries):
                    ...
                    if looks_like_plain_name(prev_line):
                        ...
                        last_added = None
            result["BuyerStructuredCompanyLines"].append(company_name)
            last_added = ("company", len(result["BuyerStructuredCompanyLines"]) - 1)
            continue
        ...
        if has_care_of:
            ...
            if not looks_like_address(line):
                result["BuyerStructuredContactLines"].append(line)
                last_added = ("contact", len(result["BuyerStructuredContactLines"]) - 1)
                continue
    ...
    if address_entries:
        result["BuyerStructuredAddressLines"] = [
