...
def is_same_as_buyer(text, buyer_name):
    """Check if the text is essentially the same as the buyer name."""
    if DEBUG:
        debug_print(f"Checking if '{text}' is same as buyer '{buyer_name}'", 1)
    
    if not text or not buyer_name:
        if DEBUG:
    ...
    buyer_norm = ' '.join(buyer_name.replace('\xa0', ' ').split()).lower()
    
    # Direct match
    if text_norm == buyer_norm:
        if DEBUG:
    ...
    buyer_no_phone = re.sub(phone_pattern, '', buyer_norm).strip()
    
    # Check again after phone removal
    if text_no_phone == buyer_no_phone:
        if DEBUG:
            debug_print("Match after phone removal", 2)
        return True
        
    # Check if one contains the other completely
    if text_no_phone in buyer_no_phone or buyer_no_phone in text_no_phone:
        # Make sure it's a substantial match (not just a short substring)
        shorter = min(len(text_no_phone), len(buyer_no_phone))
        longer = max(len(text_no_phone), len(buyer_no_phone))
    ...
    if DEBUG:
        debug_print("Not the same as buyer", 2)
    return False
...
def parse_buyer_alternate_names(soup):
    """
    Extract alternate buyer names from the Transferee(s) section.
    Returns a dict with BuyerAlternateName1, BuyerAlternateName2, and BuyerAlternateName3.
    """
    if DEBUG:
        debug_print("Starting parse_buyer_alternate_names")
    ...
    # Step 1: Locate the 'Transferee(s)' font tag
    transferee_tag = soup.find('font', string=re.compile(r'Transferee\(s\)'))
    if not transferee_tag:
        if DEBUG:
            debug_print("No Transferee(s) tag found")
        return result

    # Step 2: Find the first <br> after the 'Transferee(s)' font tag
    br_tag = transferee_tag.find_next('br')
    if not br_tag:
        if DEBUG:
            debug_print("No <br> tag after Transferee(s)")
        return result
    ...
    # Step 4: Extract alternate names (skip first node as it's the buyer name)
    for offset in range(1, 7):
        key = f"BuyerAlternateName{offset}"
