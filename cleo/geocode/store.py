...
Store format (data/coordinates.json):
{
  ...
  "addresses": {
    "NORMALIZED ADDRESS": {
      ...
      "here":     {"lat": ..., "lng": ..., "accuracy": ..., "geocoded_at": ...},
...
from cleo.config import COORDINATES_PATH, GEOCODE_CACHE_PATH, BRANDS_DATA_DIR
...
class CoordinateStore:
    ...
    def ensure_address(self, address: str) -> None:
        """Ensure an address key exists in the store (empty entry if new)."""
    ...
    def seed_scraper_coords(self) -> int:
        ...
        """
        imported = 0
        if not BRANDS_DATA_DIR.is_dir():
            return 0
        for path in sorted(BRANDS_DATA_DIR.glob("*.json")):
            data = json.loads(path.read_text(encoding="utf-8"))
            for store_rec in data:
                lat = store_rec.get("lat")
                lng = store_rec.get("lng")
                if lat is None or lng is None:
                    continue
                ...
                entry = {
                    "lat": lat,
                    "lng": lng,
                    "source": store_rec.get("brand", ""),
                    "scraped_at": store_rec.get("scraped_at", ""),
                }
    ...
    # --- Batch add methods per provider ---
    ...
    def add_mapbox_batch(
        self, addresses: list[str], results: list[Optional[dict]]
    ) -> int:
        """Merge a batch of Mapbox results into the store.
    ...
    def add_here_batch(
        ...
        """Merge a batch of HERE results into the store.
        ...
        for addr, result in zip(addresses, results):
            ...
            if mc:
                entry["match_code"] = mc
            self.set_provider(addr, "here", entry)
            added += 1
        return added
