...
function globalFilterFn(
  row: { original: TransactionSummary },
  _columnId: string,
  filterValue: string,
): boolean {
...
const columns = [
    ...
    cell: (info) => (
      <div>
        <div className="text-sm font-medium flex items-center gap-1.5 min-w-0">
          ...
          <MapLink address={`${info.getValue()}, ${info.row.original.city}`} />
        </div>
  ...
  columnHelper.accessor("sale_date", {
    ...
    size: 130,
    sortingFn: (a, b) =>
      (a.original.sale_date_iso || "").localeCompare(
        b.original.sale_date_iso || ""
      ),
    filterFn: (row, _columnId, filterValue: { from: string; to: string }) => {
      const iso = row.original.sale_date_iso || "";
      if (filterValue.from && iso < filterValue.from + "-01-01") return false;
      if (filterValue.to && iso > filterValue.to + "-12-31") return false;
      return true;
    },
    cell: (info) => (
      <div>
        ...
        <div className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1.5">
          <span className="font-mono">{info.row.original.rt_id}</span>
  ...
  columnHelper.accessor("sale_price", {
    ...
    size: 140,
    sortingFn: (a, b) =>
      parsePriceNumeric(a.original.sale_price) -
      parsePriceNumeric(b.original.sale_price),
    filterFn: (row, _columnId, filterValue: { min: string; max: string }) => {
      const price = parsePriceNumeric(row.original.sale_price);
    ...
    cell: (info) => (
      <div>
        <div className="text-sm font-medium">{info.getValue()}</div>
        {info.row.original.ppsf && (
          <div className="text-xs text-muted-foreground">{info.row.original.ppsf}/sf</div>
        )}
      </div>
    ),
  }),
...
export default function TransactionsPage() {
  ...
  } = useTableParams([{ id: "sale_date", desc: true }]);
  ...
  const brandOptions: MultiSelectOption[] = useMemo(() => {
    const brandsInData = new Set(data.flatMap((t) => t.brands));
  ...
  const columnFilters: ColumnFiltersState = useMemo(() => {
    ...
    if (yearFrom || yearTo) {
      filters.push({ id: "sale_date", value: { from: yearFrom, to: yearTo } });
    }
    if (priceMin || priceMax) {
      filters.push({ id: "sale_price", value: { min: priceMin, max: priceMax } });
    }
    if (popMin || popMax) {
